...
test = Biostrings::readDNAStringSet("../../reference_genomes/TriTrypDB-61_TcruziBrazilA4_AnnotatedCDSs.fasta") %>% 
  ...
  mutate(gene_name = str_extract(name , "^.{17}")) %>% 
...
dat = left_join(surf_beds %>% select(-start, -end, -strand) 
                , test, by = c("contig" = "gene_name" )) %>% select(-contig, -gene_name, -chr_num, -name) %>% 
...
for (file in filelist) {
  ...
  #Create df
  dat = read_delim(file, col_names = F, show_col_types = FALSE) %>% 
    mutate(MGF = surf_prot_name ) %>% 
    mutate(strain = strain_name) %>% 
    rename( gene_name = X1, start = X2, end = X3,strand = X4)
...
#change the contig name 
# get the gene id thing from the cds fasta uploaded to tritrip
test = Biostrings::readDNAStringSet("../../reference_genomes/TriTrypDB-64_TcruziBerenice_AnnotatedCDSs.fasta") %>% 
  as.data.frame() %>% rownames_to_column(var = "name") %>% select(name) %>% 
  mutate(gene_name = str_extract(name , "^.{15}")) %>% 
...
#merge and replace.
dat = left_join(surf_beds %>% select(-start, -end, -strand, -contig) 
                , test, by = c( "gene_name" ))%>% 
  select(-name) %>% 
  group_by(contig, start,end) %>% unique() %>% 
  mutate(rep = rep(1:n())) %>% 
