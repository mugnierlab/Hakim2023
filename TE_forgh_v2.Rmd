...
## Each family, grouped by type, length
```{r}
TE %>% 
  filter(classification != "Unknown") %>% 
  #make the variables to graph
  group_by(name) %>% 
  mutate(mean_family_len = mean(element_len)) %>% 
  mutate(count = table(name)) %>% 
  mutate(se_family_len = sd(element_len)/sqrt(count)) %>% 
  #reoder TE family variable so it goes in order of superfamily
  mutate(name = fct_reorder(name, classification )) %>% # this didnt work, got back to it
  #now graph
  ggplot(aes(x=name, color = classification, y = mean_family_len)) +
  geom_errorbar(aes(x= name, ymin = mean_family_len-se_family_len, ymax = mean_family_len+se_family_len))+
  geom_point(size = 4)+
  facet_wrap(~classification, scales= "free_x", nrow = 1)+
  ggtitle("length")
```

## Family size (ie number of copies in genome), grouped by super family
```{r}
TE %>% 
  filter(classification != "Unknown") %>% 
  group_by(name) %>% mutate(count = table(name)) %>% 
  ggplot()+geom_point(aes(x=name, color = classification, y= count))+
  facet_wrap(~classification, scales= "free_x", nrow = 1)+
  ggtitle("number of copies")
