...
# etc etc 
# then manuallly put them in an csv
```
...
```{bash eval=FALSE, include=T}
 ~/bbmap/sendsketch.sh records=1 mode=sequence format=3 consensus.fasta > send_sketch.out
```
...
# Clean Assembly 
## rename
Canu gives really nice descriptive names, but that's problematic long term
```{bash eval=FALSE, include=T}
cd blah blah
...
cds1 = Biostrings::readDNAStringSet("../../reference_genomes/TriTrypDB-61_TcruziCLBrenerEsmeraldo-like_AnnotatedCDSs.fasta")  %>% 
  as.data.frame() %>% 
  mutate(length = nchar(x)) %>% 
  select(-x) %>% mutate(strain = "CLBrenEsm")
cds2 = Biostrings::readDNAStringSet("../../reference_genomes/TriTrypDB-61_TcruziBrazilA4_AnnotatedCDSs.fasta")  %>% 
  as.data.frame() %>% 
  mutate(length = nchar(x)) %>% 
  select(-x) %>% mutate(strain = "BrazilA4")
cds3 = Biostrings::readDNAStringSet("../../reference_genomes/TriTrypDB-61_TcruziBrazilA4_AnnotatedCDSs.fasta")  %>% 
  as.data.frame() %>% 
  mutate(length = nchar(x)) %>% 
...
rbind(cds_merge %>%
  ...
  geom_histogram(position = "identity", alpha = 1,  color = "black") +scale_x_log10() +
  ggtitle("Predicted CDS lengths of whole genomes")+
